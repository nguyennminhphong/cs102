...
class GameConfig:
    ...
    INGAME_MUSIC_VOLUME: float = 0.50
    SOUND_EFFECT_VOLUME: float = 0.50


class LevelLoadingBarConfig:
    WIDTH: int = 600
    HEIGHT: int = 100
    STEP = 10 if not GameConfig.DEBUG else 20  # how fast the loading bar goes
...
class PlayerConfig:
    ...
    INITIAL_HP: int = 5
    INVULNERABLE_DURATION_MS: int = 100
...
class PlayerBulletConfig:
    ...
    TTL_MS: int = 1000 * 1000 // GameConfig.FPS
...
class NpcConfig:
    ...
    default_alpha: int = 255  # 180 is fully opaque
